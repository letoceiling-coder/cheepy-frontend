...
/** Удаляет markdown/рекламный мусор/цену и оставляет только безопасный текст (+<strong>). */
function normalizeAgentDescription(raw: string): string {
  let t = (raw || "").trim();
  if (!t) return t;

  // 1) Нормализуем переносы и вырезаем URL.
  t = t.replace(/\r\n?/g, "\n");
  ...
  // 3) Удаляем markdown-символы.
  t = t.replace(/[`*_#]/g, "");
  t = t.replace(/^\s*[-+>]+\s*/gm, "");

  // 4) Убираем служебные/рекламные фразы и цену (целиком предложение/строку).
  ...
  // 5) Разрешаем только <strong>, остальные теги убираем.
  t = t.replace(/<(?!\/?strong\b)[^>]*>/gi, "");

  // 6) Схлопываем пустые строки.
  t = t.replace(/\n{3,}/g, "\n\n").trim();
