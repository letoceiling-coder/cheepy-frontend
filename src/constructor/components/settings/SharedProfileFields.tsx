...
export function HotDealsSettingsField({
  ...
  const activeProductsCount = (schedule.windows ?? []).reduce((sum, w) => sum + (w.dealItems ?? []).filter((x) => x.enabled !== false).length, 0);
    ...
    <div className="space-y-3">
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              {schedule.enabled
                ? `Окон: ${schedule.windows.length}; товаров в окнах: ${activeProductsCount}`
                : `Расписание выключено; товаров в окнах: ${activeProductsCount}`}
            </p>
          </div>
      ...
      {activeProductsCount === 0 ? (
        <p className="rounded-md border border-dashed p-3 text-[11px] text-muted-foreground">
          Откройте календарь, создайте окно показа и добавьте товары внутри него. У каждого окна свои товары, скидки и таймеры.
        </p>
      ) : null}
    </div>
  );
